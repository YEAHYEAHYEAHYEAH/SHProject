...
void function(int twists, int length, int boxDim) {
  int N=length;              // DNA length
  int nTw = twists;       // Number of twists to add
  int dnatype;            // atom type for DNA

  double lx = boxDim;
  double ly = boxDim;
  double lz = boxDim;    // box size
  double density,     // density of DNA beads
    S,                // radius of circle
  ...
  sprintf(fn,"%dbeads%dtwist.in",length,twists);
  vector<atom> atoms;
  vector<bond> bonds;
  vector<angle> angles;
