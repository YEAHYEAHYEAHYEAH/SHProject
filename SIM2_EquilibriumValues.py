...
for fname in sorted(glob.glob('LongMain/*.txt')):
    ...
    plotArray = np.genfromtxt(dataFile,delimiter=',',skip_header=15003)
    ...
    corrTime = autoCorrelation(twists,100)
    
    totalLk  = (np.average(total))
    avTw     = np.average(twists)
    stddevTw = np.std(twists)
    errTw    = computeError(twists,corrTime)
    resFile.write("{},{},{},{},{}\n".format(totalLk,rigidity,avTw,stddevTw,errTw))
